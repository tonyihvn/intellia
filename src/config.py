        ...
        try:
            ...
            if os.path.exists(cls.LLM_CONFIG_FILE):
                with open(cls.LLM_CONFIG_FILE, 'r') as f:
                    ...
                    # Get the providers from loaded config
                    loaded_providers = loaded_config.get('providers', {})
                    if not isinstance(loaded_providers, dict):
                        raise ValueError("Invalid providers format")
                    
                    # Update each provider's settings from loaded config
                    for provider, settings in loaded_providers.items():
                        if provider in config['providers']:
                            # Preserve API URL from defaults
                            api_url = config['providers'][provider]['api_url']
                            config['providers'][provider].update(settings)
                            # Restore API URL
                            config['providers'][provider]['api_url'] = api_url
